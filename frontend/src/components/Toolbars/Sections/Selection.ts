...
export default (components: OBC.Components, world?: OBC.World) => {
  ...
  const onToggleVisibility = () => {
    const selection = highlighter.selection.select;    //hier werden alle Objekte mit diesem Marker ausgewählt
    if (Object.keys(selection).length === 0) return;// das Problem dabei ist verschiende Gruppen dabei zusammen geworfen werden
    for (const fragmentID in selection) {            // und nicht nur die ein nach Datenbank 
